...
> **主题：字体与可读性优化（Typography & Readability）**
> 本版本为纯视觉打磨迭代，不涉及业务逻辑与后端接口变更，可安全回滚至 `v0.2`。

### Changed（变更）
- **引言排版全面升级**：重构大标题下方中文引言区域，从单一 `.tagline` 段落
  拆分为语义化的「核心引言 + 行动口号」两段式结构（`.tag-lead` / `.tag-slogan`）。
- **字体族**：优先调用高清晰度无衬线黑体 `PingFang SC` / `Inter`，回退至 `Noto Sans SC`，
  改善暗黑背景下中文小字号的发虚与黏连问题。
- **字间距（Tracking）**：核心引言 `letter-spacing` 提升至 `.04em`，
  行动口号提升至 `.12em`，汉字间距更清爽。
- **行高（Leading）**：核心引言 `line-height` 由 `1.6` 撑大到 `1.8`，增加行间呼吸感。
- **对比度（Contrast）**：正文颜色由偏暗的 `--muted` 提升为高透光质感亮灰 `#E5E7EB`；
  口号行保持次级灰度，形成清晰的主次层级。

### Added（新增）
- **重点词高亮**：「阴阳怪气与字面噪声」「真实意图与心理动机」改用荧光绿
  （`#39FF14` / acid）下划线强调（`text-underline-offset` + 半透明描边），
  替代原先的加粗，质感更精致。

### Notes（说明）
- 宽屏（`≥768px`）首句继续保持单行平铺不折行；移动端保持自然折行美感（沿用 v0.2 行为）。
...
[0.3.0]: https://github.com/Sutmra/soultranslator/compare/v0.2.0...feat/v0.3-typography
