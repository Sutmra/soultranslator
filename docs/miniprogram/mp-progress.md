...
Step 1~2（脚手架 + 统一 request）已完成并验证。

**开工前**：`git checkout main && git pull` 确保拿到已合并的 bootstrap，再 `git checkout -b feat/mp-step3-ui` 从最新 main 开始。
（bootstrap 是否已合并 / 有无在飞 PR，用 `gh pr list` 查，不在此记录。）
